...
function(a) {
  ...
  function g(a, b) {
    ...
    for (var f in a) {
      var g = f.split("*");
      if (g.length > 2) throw new TypeError("Only one wildcard in a path is permitted");
      if (1 == g.length) {
        if (b == f) {
          d = f;
          break
  ...
  var k, l = Array.prototype.indexOf || function(a) {
  ...
  } else if ("undefined" != typeof process && process.cwd) m = "file://" + (j ? "/" : "") + process.cwd() + "/", j && (m = m.replace(/\\/g, "/"));
  else {
  ...
  ! function() {
    ...
    function h(a, b, c, d) {
      return new Promise(function(e) {
        ...
        var c;
        if (a.modules[b]) return c = f(b), c.status = "linked", c.module = a.modules[b], c;
        for (var d = 0, e = a.loads.length; e > d; d++)
          if (c = a.loads[d], c.name == b) return c;
    ...
    function m(b, d, e) {
      e.then(function(e) {
        return "loading" == d.status ? Promise.resolve(b.loaderObj.translate({
          ...
          if (void 0 === e) return d.address = d.address || "<Anonymous Module " + ++F + ">", d.isDeclarative = !0, q.call(b.loaderObj, d).then(function(b) {
            var e = a.System,
              f = e.register;
          ...
          if ("object" != typeof e) throw TypeError("Invalid instantiate return value");
          d.depsList = e.deps || [], d.execute = e.execute, d.isDeclarative = !1
        }).then(function() {
          d.dependencies = [];
          for (var a = d.depsList, c = [], e = 0, f = a.length; f > e; e++)(function(a, e) {
            c.push(h(b, a, d.name, d.address).then(function(b) {
              if (d.dependencies[e] = {
                  key: a,
                  value: b.name
                }, "linked" != b.status)
                for (var c = d.linkSets.concat([]), f = 0, g = c.length; g > f; f++) p(c[f], b)
            }))
          })(a[e], e);
          return Promise.all(c)
        }).then(function() {
          d.status = "loaded";
          for (var a = d.linkSets.concat([]), b = 0, c = a.length; c > b; b++) s(a[b], d)
        }) : void 0
      })["catch"](function(a) {
        d.status = "failed", d.exception = a;
        for (var b = d.linkSets.concat([]), c = 0, e = b.length; e > c; c++) t(b[c], d, a)
      })
    }
    ...
    function n(a) {
      return function(b) {
        var c = a.loader,
          d = a.moduleName,
          e = a.step;
        if (c.modules[d]) throw new TypeError('"' + d + '" already exists in the module table');
        for (var g, h = 0, k = c.loads.length; k > h; h++)
          if (c.loads[h].name == d) return g = c.loads[h], "translate" != e || g.source || (g.address = a.moduleAddress, m(c, g, Promise.resolve(a.moduleSource))), g.linkSets[0].done.then(function() {
            b(g)
          });
        var l = f(d);
        l.metadata = a.moduleMetadata;
        var n = o(c, l);
        c.loads.push(l), b(n.done), "locate" == e ? i(c, l) : "fetch" == e ? j(c, l, Promise.resolve(a.moduleAddress)) : (l.address = a.moduleAddress, m(c, l, Promise.resolve(a.moduleSource)))
      }
    }
    ...
    function p(a, b) {
      for (var c = 0, d = a.loads.length; d > c; c++)
        if (a.loads[c] == b) return;
      a.loads.push(b), b.linkSets.push(a), "loaded" != b.status && a.loadingCount++;
      for (var e = a.loader, c = 0, d = b.dependencies.length; d > c; c++) {
        var f = b.dependencies[c].value;
        if (!e.modules[f])
          for (var g = 0, h = e.loads.length; h > g; g++)
            if (e.loads[g].name == f) {
              p(a, e.loads[g]);
              break
            }
    ...
    function t(a, c, d) {
      var e = a.loader;
      c ? (c && a.loads[0].name != c.name && (d = b(d, "Error loading " + c.name + " from " + a.loads[0].name)), c && (d = b(d, "Error loading " + c.name))) : d = b(d, "Error linking " + a.loads[0].name);
      for (var f = a.loads.concat([]), g = 0, h = f.length; h > g; g++) {
        var c = f[g];
        e.loaderObj.failed = e.loaderObj.failed || [], -1 == l.call(e.loaderObj.failed, c) && e.loaderObj.failed.push(c);
        var i = l.call(c.linkSets, a);
        if (c.linkSets.splice(i, 1), 0 == c.linkSets.length) {
          var j = l.call(a.loader.loads, c); - 1 != j && a.loader.loads.splice(j, 1)
        }
      }
      a.reject(d)
    }
    ...
    function u(a, b) {
      ...
      var d = l.call(a.loads, b); - 1 != d && a.loads.splice(d, 1);
      for (var e = 0, f = b.linkSets.length; f > e; e++) d = l.call(b.linkSets[e].loads, b), -1 != d && b.linkSets[e].loads.splice(d, 1);
    ...
    function x(a, b, c) {
      if (c[a.groupIndex] = c[a.groupIndex] || [], -1 == l.call(c[a.groupIndex], a)) {
        ...
        for (var d = 0, e = b.length; e > d; d++)
          for (var f = b[d], g = 0; g < a.dependencies.length; g++)
            if (f.name == a.dependencies[g].value) {
              var h = a.groupIndex + (f.isDeclarative != a.isDeclarative);
              if (void 0 === f.groupIndex || f.groupIndex < h) {
                if (void 0 !== f.groupIndex && (c[f.groupIndex].splice(l.call(c[f.groupIndex], f), 1), 0 == c[f.groupIndex].length)) throw new TypeError("Mixed dependency cycle detected");
                f.groupIndex = h
              }
              x(f, b, c)
            }
    ...
    function y(a, b) {
      ...
      if (a.loads.length) {
        ...
        for (var f = e.isDeclarative == d.length % 2, g = d.length - 1; g >= 0; g--) {
        ...
        var e = b.module = z(b.name, d),
          ...
          g = b.declare.call(a, function(a, b) {
            ...
            for (var d = 0, g = e.importers.length; g > d; d++) {
              var h = e.importers[d];
              if (!h.locked)
                for (var i = 0; i < h.dependencies.length; ++i) h.dependencies[i] === e && h.setters[i](f)
            }
            return e.locked = !1, b
          });
        e.setters = g.setters, e.execute = g.execute;
        for (var h = 0, i = b.dependencies.length; i > h; h++) {
          var j = b.dependencies[h].value,
            k = d.modules[j];
          if (!k)
            for (var l = 0; l < c.length; l++) c[l].name == j && (c[l].module ? k = z(j, d) : (A(c[l], c, d), k = c[l].module));
    ...
    function E(a, c, d) {
      if (!a.evaluated && a.dependencies) {
        c.push(a);
        for (var e, f = a.dependencies, g = 0, h = f.length; h > g; g++) {
          var i = f[g];
          if (i && -1 == l.call(c, i) && (e = E(i, c, d))) return e = b(e, "Error evaluating " + i.name)
        }
        if (a.failed) return new Error("Module failed execution.");
        if (!a.evaluated) return a.evaluated = !0, e = B(a), e ? a.failed = !0 : Object.preventExtensions && Object.preventExtensions(a.module), a.execute = void 0, e
      }
    }
    var F = 0;
    e.prototype = {
      ...
      "import": function(a, b) {
        ...
        var c = this;
        return Promise.resolve(c.normalize(a, b)).then(function(a) {
          var b = c._loader;
          return b.modules[a] ? (D(b.modules[a], [], b._loader), b.modules[a].module) : b.importPromises[a] || w(c, a, g(b, a, {}).then(function(c) {
            return delete b.importPromises[a], C(b, c)
          }))
        })
      },
      load: function(a) {
        return this._loader.modules[a] ? (D(this._loader.modules[a], [], this._loader), Promise.resolve(this._loader.modules[a].module)) : this._loader.importPromises[a] || w(this, a, g(this._loader, a, {}))
      },
      module: function(a, b) {
        var c = f();
      ...
      newModule: function(a) {
        ...
        var b, c = new d;
        if (Object.getOwnPropertyNames && null != a) b = Object.getOwnPropertyNames(a);
        else {
          b = [];
          for (var e in a) b.push(e)
        }
        for (var f = 0; f < b.length; f++)(function(b) {
          k(c, b, {
            configurable: !1,
            enumerable: !0,
            get: function() {
      ...
      set: function(a, b) {
        if (!(b instanceof d)) throw new TypeError("Loader.set(" + a + ", module) must be a module");
        this._loader.modules[a] = {
          module: b
        }
      },
  ...
  var p, q = function() {
    function b(b) {
      ...
      return Promise.resolve(a["typescript" == d.transpiler ? "ts" : d.transpiler] || (d.pluginLoader || d)["import"](d.transpiler)).then(function(a) {
        ...
        var e;
        return e = a.Compiler ? c : a.createLanguageService ? g : f, 'var __moduleName = "' + b.name + '";' + e.call(d, b, a) + "\n//# sourceURL=" + b.address + "!transpiled"
      })
    }
    ...
    function f(a, b) {
      ...
      return c.modules = "system", c.sourceMap = "inline", c.inputSourceMap = a.metadata.sourceMap, c.filename = a.address, c.code = !0, c.ast = !1, b.transform(a.source, c).code
    }

    function g(a, b) {
      var c = this.typescriptOptions || {};
      return void 0 === c.target && (c.target = b.ScriptTarget.ES5), c.module = b.ModuleKind.System, c.inlineSourceMap = !0, b.transpile(a.source, c, a.address)
    }
    return e.prototype.transpiler = "traceur", b
  }();
  h.prototype = e.prototype, f.prototype = new h;
  var r = /^([^\/]+:\/\/|\/)/;
  f.prototype.normalize = function(a, b) {
  ...
  var s;
  if ("undefined" != typeof XMLHttpRequest) s = function(a, b, c) {
    function d() {
      b(f.responseText)
    }
    ...
    if (!("withCredentials" in f)) {
      var j = /^(\w+:)?\/\/([^\/]+)/.exec(a);
      j && (g = j[2] === window.location.host, j[1] && (g &= j[1] === window.location.protocol))
    }
    ...
    var t;
    s = function(a, b, c) {
      if ("file:///" != a.substr(0, 8)) throw "Only file URLs of the form file:/// allowed running in Node.";
      return t = t || require("fs"), a = j ? a.replace(/\//g, "\\").substr(8) : a.substr(7), t.readFile(a, function(a, d) {
        if (a) return c(a);
        var e = d + "";
        "\ufeff" === e[0] && (e = e.substr(1)), b(e)
      })
    }
  }
